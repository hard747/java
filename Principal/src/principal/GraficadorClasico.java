...
public class GraficadorClasico extends JPanel
{
    ...
    public GraficadorClasico(Container Contenedor)
    {
        ...
        campoIntervaloA = new JTextField("-5",3);
        campoIntervaloB = new JTextField("5",3);
        campoNumeroDeInteraciones = new JTextField("5",3);
        campoNoPuntos = new JTextField("99",3);
        campoFuncion = new JTextField("cos(x)",15);//x^3+2x^2+7x-20//cos(x)
        ...
        rotulo = BorderFactory.createTitledBorder(" Metodos ");
...
public class ZonaGrafica extends JPanel  implements MouseListener, MouseMotionListener, MouseWheelListener
{
    ...
    void Graficar(Graphics ap, int xg, int yg)
    {
        ...
        if(!errorEnExpresion)
        {
            Mensaje.setText("metodos numericos");//:. Arrastre el mouse para mover ejes.");
